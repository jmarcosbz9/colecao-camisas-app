<template>
  <div v-if="open" class="fc-backdrop" @mousedown.self="close">
    ...
    >
      ...
      >
        <div class="fc-head-left">
          ...
          <h3 class="fc-title">Editar Manto</h3>
        </div>
        <button class="fc-close" type="button" @click="close">×</button>
      </div>

      <form class="fc-body" @submit.prevent="submitUpdate" spellcheck="false">
        ...
        <div class="row one">
          <div class="field">
            ...
            <!-- Thumbs existentes (maiores que no grid) -->
            <div v-if="existingFotos.length" class="ex-wrap">
              <div class="ex-title">Fotos existentes (clique para ver)</div>
              <div class="ex-grid">
                <button
                  v-for="(f, i) in existingFotos"
                  :key="'ex-' + i"
                  type="button"
                  class="ex-item"
                  @click.stop="openExistingViewer(i)"
                >
                  <img class="ex-img" :src="fotoThumbSrc(f)" alt="" />
                </button>
              </div>
            </div>
            <div v-else class="ex-empty">(sem fotos)</div>
    ...
    <PhotoViewerUpdate
      ...
      :startIndex="pvStart"
      @close="pvOpen = false"
      @confirm-delete="onConfirmDeletePv"
    />
</form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      // Fotos existentes (do DB) e deleções pendentes (executa no Salvar)
      existingFotos: [],
      pendingDelete: [],

      // PhotoViewerUpdate (somente para fotos EXISTENTES)
      pvOpen: false,
      pvStart: 0,
  ...
  watch: {
    ...
    item: {
      handler() {
        // Se trocar o registro enquanto o modal está aberto
        if (this.open) this.applyItemToForm()
      },
      deep: false,
    },
  },
  ...
  methods: {
    onOpen() {
      ...
      // prepara o form com os dados do registro atual
      this.applyItemToForm()
    },

    applyItemToForm() {
      // limpa erros e fotos locais (novas)
      this.errs = { Modelo: '', Fornecedora: '', Manga: '', Conservacao: '', Periodo: '', Numero: '', Preco: '', Observacoes: '', Fotos: '', _global: '' }
      this.photoItems = []
      this.dzOver = false


      // fotos existentes vindas do backend (para exibir thumbs no Update)
      this.existingFotos = this.parseFotosFromItem(this.item)
      this.pendingDelete = []
      this.pvOpen = false
      this.pvStart = 0
      const it = this.item || null
      const id = it && (it.Id ?? it.id ?? it.Codigo)
      this.editId = (id != null) ? id : null

      // replica a estrutura do Create e preenche somente o que existir
      const next = {
        Modelo: (it && it.Modelo) ? String(it.Modelo) : '',
        Fornecedora: (it && it.Fornecedora) ? String(it.Fornecedora) : '',
        Manga: (it && it.Manga) ? String(it.Manga) : '',
        Conservacao: (it && it.Conservacao) ? String(it.Conservacao) : '',
        Periodo: (it && (it.Periodo || it.Período)) ? String(it.Periodo || it.Período) : '',
        Numero: (it && it.Numero != null) ? String(it.Numero) : '',
        Preco: (it && it.Preco != null) ? String(it.Preco) : '',
        Observacoes: (it && it.Observacoes) ? String(it.Observacoes) : '',
        fornecedora_id: (it && it.fornecedora_id != null) ? it.fornecedora_id : null,
      }

      // Atualiza refs reativas (mantém o objeto form)
      for (const k in next) {
        if (Object.prototype.hasOwnProperty.call(next, k)) this.form[k] = next[k]
      }


      // inicializa o campo VISÍVEL de preço (evita nascer 0,00)
      try {
        this.precoDisplay = this.formatPrecoToBR(this.form.Preco || '0.00')
      } catch (e) {
        this.precoDisplay = '0,00'
      }
      // tenta resolver fornecedora_id por nome (se vier vazio)
      if (next.Fornecedora && !next.fornecedora_id && Array.isArray(this.fornecedoras) && this.fornecedoras.length) {
        const alvo = String(next.Fornecedora || '').toLowerCase()
        const hit = this.fornecedoras.find(f => String(f && f.nome || '').toLowerCase() === alvo)
    ...
    async fetchFornecedoras() {
      try {
        const r = await axios.get('/api/fornecedoras?contexto=flamengo');
    ...
    async submitUpdate() {
      ...
      if (this.editId == null) {
        this.setErr('_global', 'Registro inválido para edição.')
        return
      }
      if (!this.validate()) {
    ...
    // ===== Fotos existentes (Update) =====
    parseFotosFromItem(it){
      try{
        if(!it) return []
        const v = it.Fotos
        if(!v) return []
    ...
    buildPvItems(){
      ...
      return a.map(f => ({ fullSrc: this.fotoFullSrc(f), thumbSrc: this.fotoThumbSrc(f), raw: f }))
    },
    openExistingViewer(idx){
      this.pvStart = idx || 0
      this.pvOpen = true
    },
    onConfirmDeletePv(it){
      const raw = it && it.raw ? it.raw : null
      if(!raw) return
      // Opção B: só marca (remoção real acontece no Salvar via backend diff old->kept)
      this.pendingDelete.push(raw)
      const name = String(raw.name || '')
      this.existingFotos = (this.existingFotos || []).filter(x => String((x && x.name) || '') !== name)
    },
...
/* Update: thumbs existentes maiores que no grid */
.ex-wrap{ margin: 6px 0 10px; }
.ex-title{ font-size: 12px; opacity: .85; margin-bottom: 6px; }
.ex-grid{ display:flex; flex-wrap:wrap; gap:10px; }
.ex-item{ padding:0; border:1px solid rgba(255,255,255,.16); background: rgba(255,255,255,.06); border-radius:10px; cursor:pointer; }
.ex-img{ width: 150px; height: 110px; object-fit: cover; display:block; border-radius:10px; }
